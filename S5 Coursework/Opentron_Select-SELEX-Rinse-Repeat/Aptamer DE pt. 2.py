# Metadata
metadata = {
    'protocolName': 'Pt 2. FULL PROTOCOL',
...
def run(protocol):
    ...
    update_well_tracking(
        ...
        overrides= {'A1': 360, 'B1': 360, 'C1': 360, 'D1': 360, 'E1': 360, 'H1': 360, 
                  'A2': 240, 'B2': 240, 'C2': 0, 'D2': 0, 'E2': 0, 'F2': 0,
                  'A3': 45, 'B3': 45, 'C3': 0, 'D3': 0, 'E3': 0, 'F3': 0}
    )
    update_well_tracking(
        tc_plate,
        capacity=200,
    ...
    # Wells for each run
    aspirate_wells = [SR2_Aptamers, SR3_Aptamers, SR4_Aptamers, SR5_Aptamers]
    dispense_wells = [R3_Aptamers, R4_Aptamers, R5_Aptamers, R6_Aptamers]

     # Number of PCR runs
    pcr_runs = 2  # Adjust this value as needed

    # Iterate over each run
    for run in range(3, pcr_runs + 3):
        ...
        for TC_well in TC_wells:
            p20.aspirate(5, aspirate_wells[run -3 ], rate = p20_aspirate_fast)
        ...
        for TC_well in TC_wells:
            ...
            p300.dispense(20, dispense_wells[run -3], rate = p300_dispense_slow)  # Dispense into run well
        p300.drop_tip()
        ...
        for selex_step in range(len(MP_volume)):
            ...
            if selex_step == 0:
                ...
                # Aspirate supernatant of both wells. # Here we have to adjust finely the height to just aspirate the supernatant
