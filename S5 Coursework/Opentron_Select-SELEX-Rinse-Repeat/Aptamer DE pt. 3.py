# Metadata
metadata = {
    'protocolName': 'Pt 3. FULL PROTOCOL',
...
def run(protocol):
    ...
    list_rows = ["A", "B", "C", "D", "E", "F", "G", "H"] #WHY? # Pablo commenting here, It is to access to the different rows with identation for each selex cycle
    ...
    update_well_tracking(
        ...
        overrides={'A1': 360, 'B1': 360, 'C1': 360, 'D1': 360, 'E1': 360, 'H1': 360, 
                  'A2': 240, 'B2': 240, 'C2': 240, 'D2': 240, 'E2': 0, 'F2': 0,  # 240uL in the first four result wells
                  'A3': 45, 'B3': 45, 'C3': 45, 'D3': 45, 'E3': 0, 'F3': 0}
    )
    update_well_tracking(
        tc_plate,
        capacity=200,
    ...
    # Wells for each run
    aspirate_wells = [SR4_Aptamers, SR5_Aptamers]
    dispense_wells = [R5_Aptamers, R6_Aptamers]

     # Number of PCR runs
    pcr_runs = 2  # Adjust this value as needed

    # Iterate over each run
    for run in range(5, pcr_runs + 5):
        ...
        for TC_well in TC_wells:
            p20.aspirate(5, aspirate_wells[run -5], rate = p20_aspirate_fast)
        ...
        for TC_well in TC_wells:
            ...
            p300.dispense(20, dispense_wells[run -5], rate = p300_dispense_slow)  # Dispense into run well
        p300.drop_tip()
        ...
        # 1. Add Milk Powder (MP)to the aptamer library.
        for selex_step in range(len(MP_volume)):
            ...
            # 2. Add magnetic beads (MB) to the same well.
            ...
            if selex_step == 0:
                ...
                # 5.1 Pull down by activating the magnets. 
                ...
                p300.dispense((library_volume + MP_volume[selex_step]) / 2, waste, rate = p300_dispense_fast) # For the recording maybe we want it on a well we can photo afterwards
                ...
                # For the the following SELEX iterations after the first one, we don't have an overflowing problem because the library was diluted in 45 µl
